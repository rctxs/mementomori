...
export class LifeYear {
  ...
  constructor(birthday: Date, yearNumber: number, firstWeekNumber: number) {
    ...
    nextYearsBirthday.setFullYear(
      nextYearsBirthday.getFullYear() + yearNumber + 1
    );
    this.weeks = [];
  ...
  isDateWithin(date: Date): boolean {
    return (
      this.weeks.slice(0)[0].monday <= date &&
      date < this.weeks.slice(-1)[0].nextMonday
    );
  }

  isWeekNrWithin(weekNumber: number): boolean {
    return (
      this.getFirstLifeWeekNr() <= weekNumber &&
      weekNumber <= this.getLastLifeWeekNr()
    );
  }
}
...
export class Life {
  ...
  getLifeWeekByDate(date: Date): LifeWeek {
    ...
    if (filteredLifeYears.length === 1) {
      let filteredLifeWeeks = filteredLifeYears[0].weeks.filter((week) =>
  ...
  getLifeWeekByNr(number: number): LifeWeek {
    ...
    if (filteredLifeYears.length === 1) {
      let filteredLifeWeeks = filteredLifeYears[0].weeks.filter(
